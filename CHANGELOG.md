...
- Added Modus Quad Engine to Object Mode at the bottom of the Modus N-panel
- Added Fast Retopology and Slow Retopology with an approximate target triangle budget.
- Added Quick Decimate using the engine's Compact QEM workflow.
- Added optional object-local X, Y, or Z symmetry, Sharp edge boundaries, quad smoothing controls, and Multiresolution/Shrinkwrap projection.
- Added background processing with live status, cancellation, automatic result import, and hidden storage for original meshes.
